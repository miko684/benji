...
import { getCredit, request, setRotatingCookieHandler } from "@/api/controllers/core.ts";
...
function mergeCookieValues(existing: string, updates: Record<string, string>): string {
  const replaced = new Set(Object.keys(updates).map((key) => key.toLowerCase()));
  const kept = existing
    .split(";")
    .map((part) => part.trim())
    .filter((part) => {
      ...
      return !replaced.has(part.slice(0, separator).trim().toLowerCase());
    });
  for (const [key, value] of Object.entries(updates)) kept.push(`${key}=${value}`);
  return kept.join("; ");
}

export function applyRotatingCookies(sessionId: string, updates: Record<string, string>): void {
  if (!sessionId || !Object.keys(updates).length) return;
  const rows = db.prepare(`SELECT * FROM ${TABLE} WHERE credential_kind = 'cookie'`).all() as AccountRow[];
  for (const row of rows) {
    const credential = decrypt(row.credential_ciphertext);
    if (extractSessionId(credential, row.credential_kind) !== sessionId) continue;
    const merged = mergeCookieValues(credential, updates);
    if (merged === credential) return;
    const timestamp = now();
    db.prepare(`UPDATE ${TABLE} SET credential_hash = ?, credential_ciphertext = ?, updated_at = ? WHERE id = ?`).run(
      hashCredential(merged), encrypt(merged), timestamp, row.id
    );
    return;
  }
}
...
export async function refreshStatus(id: string): Promise<PublicAccount> {
  ...
  try {
    ...
    const membership = membershipStatusProvider ? await membershipStatusProvider(credential, toPublic(row)) : "unknown";
    const timestamp = now();
...
setRotatingCookieHandler(applyRotatingCookies);
